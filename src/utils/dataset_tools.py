...
class HypomethylatedRegion:
    ...
    def select_dataset_col(self, select_col_name: str):
        coordinate_col_list = self.dataset_pldf.columns[0: 3]
        select_col_list = coordinate_col_list.append(select_col_name)
        self.processing_dataset_pldf = self.dataset_pldf[select_col_list]

    def mark_low_me_region(self, select_col_name: str, min_cg_num=5, threshold_min_region_len=0, low_me_value=0.25):
        self.processing_dataset_pldf = self.processing_dataset_pldf.with_columns(
            (pl.col(select_col_name) < low_me_value).alias('is_smooth_low_me')
        ).with_columns(
            ...
            pl.when(pl.col('is_smooth_low_me') &
                    (pl.col('region_cg_num') >= min_cg_num) &
    ...
    def keep_low_me_region_and_mark_represent_cg(self, interval=1000):
        ...
        for name, group_pldf in self.processing_dataset_pldf.group_by(['low_me_region_id']):
            ...
            group_pldf = group_pldf.with_columns(
                ...
                ((pl.cum_count('for_cum_count') - 1).over(['breakpoint']) == pl.col('abs(start-mid_pos)').arg_min()
                 .over(['breakpoint'])).cast(pl.Int64).alias('represent_cg')
            ).drop('for_cum_count')
