...
**Note:** Installing `bioconductor-bsseq` may require downloading sizable Bioconductor dependencies, including `GenomeInfoDb` and `GenomeInfoDbData`. If the installation fails because of network issues or interrupted downloads, configure an HTTP/HTTPS proxy and retry, or install the Bioconductor packages through R using a nearby Bioconductor mirror, as described below.
...
R --vanilla <<'EOF'
# Increase the download timeout for large dependency packages.
options(timeout = max(600, getOption("timeout")))

# Specify CRAN and Bioconductor mirrors.
options(
    repos = c(
...
# Install bsseq together with its main Bioconductor dependencies.
BiocManager::install(
    c(
        "GenomeInfoDb",
        "GenomeInfoDbData",
...
```bash
python script/preprocess/preprocess_encode_data.py \
...
- Runtime metadata are written under `data/encode_preprocess/preprocess_metadata`:
  - `preprocess.log`: runtime log generated by logger.
...
- `--output_file_format`: Output format for preprocessed data. Choose `tsv` or `feather`. Default: `tsv`.
- `--reference_cpg_coordinate_file`: Optional reference CpG coordinate BED file. If provided, methylation data are aligned to these coordinates.
- `--metadata_folder_name`: Optional metadata subfolder name. Default: `preprocess_metadata`.
- `--output_summary_file`: Optional summary filename. Default: `preprocess_summary.tsv`.
- `--output_manifest_file`: Optional manifest filename. Default: `preprocess_manifest.tsv`.
- `--output_log_file`: Optional runtime log filename. Default: `preprocess.log`.
